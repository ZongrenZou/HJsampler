...
def rhs1_rev(y, t):
    dydt = - np.sin(4*np.pi*(T - t)) - 3 * y * (1 - y)
    return dydt
...
## sampling
eps = 0.005
model = models.NN(units=50, name="nn2", eps=np.array(eps), sigma=0.05, activation=tf.tanh)
model.restore()
