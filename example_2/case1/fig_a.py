...
def check_riccati(cov1, cov2, mu1, mu2, yt, eps, T, t0):
    B = np.array(
        [
            [0, -1],
            [1, 1],
...
def check_sgm(yt, eps, T, t0):
    ...
    for i in trange(N):
        update = eps * forward(
            (T - t) * tf.ones(shape=[M, 1]),
            tf.constant(zt[:, 0:1], tf.float32),
            tf.constant(zt[:, 1:2], tf.float32),
        ).numpy()
        b = (-B @ zt.T).T
        zt = zt + (update - b) * dt + \
            np.sqrt(eps * dt) * np.random.normal(size=zt.shape)
        t = t + dt 
    return zt
...
zt_sgm = check_sgm(
    yt=yt, eps=eps, T=T, t0=t0,
)

sio.savemat(
    "./outputs/case_4.mat",
    {
        "samples_riccati": zt_riccati,
        "samples_sgm": zt_sgm,
    }
)